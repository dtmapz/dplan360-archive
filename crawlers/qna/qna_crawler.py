...
def main():
    ...
    sheet_id = os.getenv("QNA_SHEET_ID", "").strip()
    ...
    if not login_id or not login_pw or not gsheet_creds_json or not sheet_id:
        logger.error(
            "필수 환경 변수 누락: QNA_LOGIN_ID, QNA_LOGIN_PW, "
            "QNA_GCP_SERVICE_ACCOUNT_JSON, QNA_SHEET_ID"
        )
        sys.exit(1)
